...
#define closure_pool_current_base closure_pool.bases[closure_pool.next_base-1]

static inline int root_of(int l, int r) {
    return (r-l)/2 + l - 1;
}

static inline int position_of(Closure cl) {
    int p;

    for (p = closure_pool.next_base; p > 0; --p) {
	if (cl >= closure_pool.bases[p-1]
	    && cl < closure_pool.bases[p-1] + (CLOSURE_POOL_INIT_SIZE << (p-1))) {
...
SENTENCE find_an_empty_slot_as_ncl(Closure *ncl_r)
{
    int l, r;
...
Closure alloc_closure(void)
{
	...
	if (closure_pool.size == 0) {
	    ...
	    closure_pool.bases[closure_pool.next_base++] = closure_pool.closures;
	}
	if (closure_pool.label[root_of(0, closure_pool.size)]) {
	    /* Do not free memory here to keep the space from being reused for other purposes. */
	    closure_pool.size <<= 1;
	    _CALLOC(closure_pool.closures, closure_pool.size);
	    closure_pool.bases[closure_pool.next_base++] = closure_pool.closures;

	    REALLOC_ARRAY(closure_pool.label, closure_pool.size << 1);
	    memset(closure_pool.label+closure_pool.size, 0, closure_pool.size);
	}
    } while (0);
